...
$Id: TextAreaTest.cfc 2430 2010-09-20 07:05:21Z peterjfarrell $
...
--->
<cfcomponent
	displayname="TextAreaTest"
	extends="mxunit.framework.TestCase"
	hint="Test cases for 'textarea' custom tag.">
	...
	--->
	<cffunction name="testTextArea" access="public" returntype="void" output="false"
		hint="Test basic 'textarea' tag.">
		...
		<cfset bean.setLastName("The quick brown fox jumps over the lazy dog") />
		<cfset event.setArg("user", bean) />

		<cfsavecontent variable="output">
			<root>
				<form:form actionEvent="something" bind="${event.user}">
					<form:textarea path="lastName" cols="100" rows="10" />
				</form:form>
			</root>
		</cfsavecontent>
		...
		<cfset node = assertXPath('/root/form/textarea[@id="lastName"]', xml, "The quick brown fox jumps over the lazy dog") />
	</cffunction>
