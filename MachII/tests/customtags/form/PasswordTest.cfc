...
$Id: PasswordTest.cfc 2428 2010-09-20 06:19:26Z peterjfarrell $
...
<cfcomponent
	...
	hint="Test cases for 'password' custom tag.">
	...
	--->
	<cffunction name="testPassword" access="public" returntype="void" output="false"
		hint="Test basic 'password' tag.">
		...
		<cfsavecontent variable="output">
			<root>
				<form:form actionEvent="something" bind="${event.user}">
					<form:password path="favoriteColor" />
					<form:password path="lastName" showPassword="true" />
				</form:form>
			</root>
		</cfsavecontent>
		...
		<cfset node = assertXPath('/root/form/input[@type="password" and @id="favoriteColor"]', xml) />
		<cfset node = assertXPath('/root/form/input[@type="password" and @value="Farrell" and @id="lastName"]', xml) />
	</cffunction>
